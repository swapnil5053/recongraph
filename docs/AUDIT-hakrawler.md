...
Read through on 13 Sep 2026, from a fresh clone of `github.com/hakluke/hakrawler` (MIT). I wrote this up before deciding whether to fork it or start over.
...
So the whole tool is 231 lines of logic wrapped around Colly. There isn't a framework here to fork, and very little worth keeping as code.
...
**The crawl loop** (`hakrawler.go:71-199`):
...
3. `var sm sync.Map`, a package-level global used only by `isUnique`.
...
These four are the main reason I went for a bounded worker pool with explicit cancellation rather than patching.
...
The `-u` loop drains `results` to close; the unconditional loop that follows then ranges over a closed, empty channel and does nothing. It happens to produce correct output, but only by accident of ordering. The intent was clearly `if/else`.
...
**Rate limiting: none.** What exists:

- `Parallelism`, which is a concurrency cap, not a rate. 8 concurrent requests against one host is 8 concurrent requests as fast as the network allows.
...
| Connection-pool tuning | ✗ (default `Transport`, rebuilt per target anyway) |

In practice, `-t 32` against a small site is 32 unthrottled concurrent requests. A 429 is treated as an ordinary non-2xx and the URL is dropped without a retry.

---

## 5. Link discovery mechanism

I expected regex here and was wrong: hakrawler does **not** use regex for link extraction. Colly parses with `goquery`, which sits on `golang.org/x/net/html`, a real tokeniser. Extraction is CSS-selector based. That part is sound.
...
- JavaScript. `.js` files are listed but never fetched or analysed. This is the largest functional gap versus katana.
...
No file output flag, no CSV, no DOT, no database. No HTTP metadata in the output at all: **no status code, no content type, no content length, no response headers, no timing, no depth, no discovery timestamp**. Once the pipe closes, everything the crawl learned is gone. There is no run identity, so there is nothing to compare two runs against.

This is the gap I actually care about. The output is a *stream of strings*, not a *record of a crawl*.
...
- `temoto/robotstxt` is compiled in via Colly but never activated.
...
Roughly in the order I'd want them fixed.
...
4. Per-host rate limiting and jitter.
5. Per-request timeouts, redirect caps, response-size caps enforced consistently.
6. Error surfacing. `c.Visit` errors, proxy-URL parse errors, and every non-2xx are discarded silently, so there's no way to tell "no links found" from "everything 403'd".

**Data model (the reason for ReconGraph):**
7. No persistence; nothing survives the pipe.
8. No response metadata (status, type, size, headers, timing, depth, parent).
9. No relationship model: URLs, never *what linked to what*.
...
20. Single-file `package main`, so nothing is importable as a library.
...
hakrawler does one thing well: turn a URL into a list of URLs, in a way that drops neatly into a shell pipeline. That's why it has 5k stars. The code is 231 lines with four defects (goroutine leak, `recover()` as control flow, dead drain loop, scope-escape regex) and one decision I'd keep, which is parsing HTML with a real parser.

Not forking. A fork would bring a 2022 pseudo-versioned Colly pin and a `package main` I'd delete in the first commit. I'm keeping two ideas, the shape of the crawl loop and reading seeds from stdin, starting an empty module, and crediting hakrawler in the README.
