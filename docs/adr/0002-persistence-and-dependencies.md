...
**What prompted it.** I was building behind a proxy that let `github.com` through but not the `golang.org`, `gopkg.in`, `gitlab.com` or `modernc.org` import hosts. `modernc.org/sqlite`, the pure-Go driver ADR-0001 picked, pulls in about a dozen modules from those hosts, so it wouldn't build. That made me look at the storage decision earlier than planned, and I think the result is better anyway:

1. **Diff over the model beats diff over SQL.** ADR-0001 put the main feature inside the one component with a heavy external dependency. Comparing graphs in Go instead means `internal/diff` is pure, has no I/O, runs in 3ms, and is tested with ten table-driven cases and no database. It also means diff works identically across every backend, present and future. Tying it to one storage engine was the weaker design.
...
No ad-hoc SQL over crawl history. Two mitigations: the `query` subcommand covers the questions the graph is actually for (orphans, hubs, reachability, third-party surface, tech, findings), and the interface means a SQLite backend can be added later without touching `diff`, `crawl`, or `export`.
...
The helper exists because of a trap in the standard library: `flag.Parse` stops at the first non-flag token, so `recongraph query latest --orphans` silently ignores `--orphans`. I found this by hand, not with a test, and a framework would have avoided it. `parseArgs` permutes arguments so flags may appear before or after positionals. A flag that is quietly dropped is worse than one that errors.
...
**Negative.** No SQL. A future SQLite backend is real work, though the interface keeps it from being a rewrite. The `flag`-based CLI has rougher help output than Cobra's, and `parseArgs` is a workaround for a standard-library behaviour that a framework handles.

**Follow-up.** While behind that proxy, `go.mod` briefly carried a `replace` pointing `golang.org/x/net` at its GitHub mirror. It has since been removed.
